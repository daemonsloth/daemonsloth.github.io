...
const pages = {
  ...
  education: () => `
    ...
    <div class="output">
      ...
      <h2 class="section-title">Learned foundations.<br><span>Applied relentlessly.</span></h2>
      ${renderEducation()}
    </div>
  `,
...
function resolveCommand(command) {
  const cleaned = command.trim().toLowerCase().replace(/^\.\//, '');
  ...
  return {
    ...
    requestedFile,
    builtIn,
    key: fileMap[requestedFile] || (pageFileMap[cleaned] ? cleaned : builtIn) || 'home'
  };
}
